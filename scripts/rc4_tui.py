...
def draw(stdscr, selected, status, result, focus, language="es"):
    ...
    add_box(stdscr, 1, 1, height - 3, left_w, tr(language, "nav")); nav = [tr(language, "dash"), tr(language, "machine"), tr(language, "intent"), tr(language, "rec"), tr(language, "evidence")]
    put(stdscr, 3, 4, "[X] LEONES RC4", left_w - 6)
    for i, item in enumerate(nav): put(stdscr, 4 + i, 6, "-> " + item, left_w - 8)
    put(stdscr, 10, 4, "[ ] " + tr(language, "legacy"), left_w - 6); put(stdscr, 12, 4, "[ ] " + tr(language, "settings"), left_w - 6); put(stdscr, 14, 4, "[ ] " + tr(language, "exit"), left_w - 6)
    ...
    for idx, (key, label) in enumerate(PURPOSES): put(stdscr, bottom_y + 5 + idx, inner_x, f"{'>' if idx == focus else ' '} [{'X' if key in selected else ' '}] {idx + 1}. {label}", content_w)
    ...
    put(stdscr, height - 1, 2, tr(language, "footer"), width - 4); stdscr.refresh()


def main():
    selected = set(); focus = 0; status = "ready"; result = None
    def app(stdscr):
        ...
        while True:
            draw(stdscr, selected, status, result, focus, language); key = stdscr.getch()
            ...
            if key in (curses.KEY_UP, ord("k")): focus = (focus - 1) % len(PURPOSES)
                ...
                status = "running"; draw(stdscr, selected, status, None, focus, language); status, result = run_recommendation([p for p, _ in PURPOSES if p in selected])
