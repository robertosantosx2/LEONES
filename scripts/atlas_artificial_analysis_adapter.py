...
METRIC_ALIASES = {
    ...
    "tok/s": "throughput",
    "cost_per_million_tokens": "cost_per_1m_tokens",
    "price": "cost_per_1m_tokens",
}
...
def main():
    ...
    for r in src:
        metric = METRIC_ALIASES.get(
            (r.get("metric") or "").lower(), r.get("metric", "")
        )
        ...
        key = "|".join(
            [model, metric, value, unit, r.get("hardware", ""), r.get("runtime", "")]
        )
    ...
    print(
        f"Artificial Analysis: {added} new structured observations; {len(existing)} total evidence records"
    )
