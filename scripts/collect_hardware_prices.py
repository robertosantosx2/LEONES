...
CPU_PAT = re.compile(
    r"\b(?:Intel\s+)?Core\s+i([3579])\b|\b(?:AMD\s+)?Ryzen\s+([3579])\b", re.I
)
GPU_PAT = re.compile(
    r"\b(?:NVIDIA\s+)?(?:GeForce\s+)?(RTX\s*\d{3,4}(?:\s*(?:Ti|SUPER))?)\b", re.I
)
RAM_PAT = re.compile(
...
PRICE_PAT = re.compile(
    r"(?<!\d)(\d{1,5}(?:[.,]\d{2})?|\d{1,5}\s*\^\{\d{2}\})\s*(?:€|EUR)", re.I
)
    ...
    try:
        with urlopen(
            Request(
                url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; LEONES-HardwarePriceBot/1.0)"
                },
            ),
            timeout=35,
        ) as r:
        ...
        try:
            with urlopen(
                Request(proxy, headers={"User-Agent": "LEONES-HardwarePriceBot/1.0"}),
                timeout=60,
            ) as r:
                return r.read().decode("utf-8", "ignore"), url
        except Exception as pe:
            raise RuntimeError(f"direct={e}; jina={pe}") from pe
...
def parse_price(raw):
    ...
    if "," in s:
        s = s.replace(".", "").replace(",", ".")
    elif not (s.count(".") == 1 and len(s.rsplit(".", 1)[1]) == 2):
        s = s.replace(".", "")
    try:
        p = float(s)
    except ValueError:
...
def round10(price):
    return float(int(price / 10 + 0.5) * 10)
...
def jsonld_products(text):
    ...
    for raw in re.findall(
        ...
        for x in items:
            ...
            try:
                p = float(str(off.get("price")).replace(",", "."))
            except Exception:
                continue
            name = clean(str(x.get("name", "")))
            if name:
                out.append((name, p, str(off.get("url", "")), name))
    return out


def text_products(text):
    lines = [
        re.sub(r"\s+", " ", unescape(x)).strip()
        for x in re.sub(r"<[^>]+>", "\n", text).splitlines()
        if x.strip()
    ]
    out = []
    ...
    for i, line in enumerate(lines):
        for pm in PRICE_PAT.finditer(line):
            ...
            candidates = [
                ...
                or GPU_PAT.search(c)
                or RAM_PAT.search(c)
                or "procesador -" in c.lower()
                or "memoria ram -" in c.lower()
                or "tarjeta gráfica -" in c.lower()
                or "tarjeta grafica -" in c.lower()
            ]
            if not candidates:
                continue
            name = clean(candidates[0])
            key = (name.lower(), round10(p))
            if key not in seen and len(name) >= 4:
                seen.add(key)
                out.append((name, p, "", " | ".join(window)))
    return out
...
def discover_links(seed, html, limit=12):
    ...
    for h in parser.links:
        u = urljoin(seed, h)
        p = urlparse(u)
        if p.netloc != base.netloc or u.startswith("mailto:"):
            continue
...
def classify(name):
    ...
    if m:
        ...
        v = "intel" if m.group(1) else "amd"
        return "cpu", v, f"Core i{f}" if v == "intel" else f"Ryzen {f}", "", ""
    m = RAM_PAT.search(n)
    if m:
        d = m.group(1) or m.group(4)
        cap = m.group(2) or m.group(3)
        return "ram", "memory", f"DDR{d}", cap, ""
    m = GPU_PAT.search(n)
    if m:
...
def valid_row(r):
    try:
        p = float(r.get("price_eur", "").replace(",", "."))
    except ValueError:
        return False
    ...
    return (
        ...
        and "###" not in m
        and 5 <= p <= 10000
        and r.get("component_type") in {"cpu", "ram", "gpu"}
    )
...
def save_obs(rows):
    ...
    with OBS.open("w", encoding="utf-8", newline="") as f:
        csv.DictWriter(f, fieldnames=FIELDS).writerows([]) if False else None
...
def build_outputs(rows):
        ...
        summary.append(
            dict(
                zip(
                    sf,
                    [
                        f"{r['component_type']}|{r['vendor']}|{r['model']}|{r['capacity_gb']}|{r['vram_gb']}",
                        r["component_type"],
                        r["vendor"],
                        r["category"],
                        r["model"],
    ...
    for k, vs in groups.items():
        ps = sorted(float(x["price_eur"]) for x in vs)
        med = (
            ps[len(ps) // 2]
            if len(ps) % 2
            else (ps[len(ps) // 2 - 1] + ps[len(ps) // 2]) / 2
        )
        market.append(
            dict(
                zip(
                    ...
                    [
                        *k,
                        len(set(x["source"] for x in vs)),
                        ";".join(sorted(set(x["source"] for x in vs))),
                        f"{min(ps):.0f}",
                        f"{med:.0f}",
                        f"{max(ps):.0f}",
                        max(x["observed_at"] for x in vs),
                    ],
                )
            )
        )
...
def main():
    ...
    for sid in order:
        conf = targets[sid]
        name = {
            "ES-PCOMP": "PcComponentes",
            "ES-COOLMOD": "Coolmod",
            "ES-MEDIAMARKT": "MediaMarkt España",
            "EU-LDLC": "LDLC España",
        }.get(sid, sid)
        se = sa = 0
        for seed in conf.get("urls", {}).values():
            try:
                html, canon = fetch(seed)
                pages = [(canon, html)]
                if conf.get("strategy") == "discover":
                    pages += [(u, fetch(u)[0]) for u in discover_links(seed, html)]
                        ...
                        rows.append(
                            {
                                ...
                                "category": cat,
                                "model": name0,
                                "capacity_gb": str(cap).strip(),
                                "vram_gb": str(vram).strip(),
                                "price_eur": f"{price:.0f}",
                                "price_type": "observed_order_of_magnitude",
                                "market": "Spain",
                                "currency": "EUR",
                                "source": name,
                                "source_url": purl or pu,
                                "notes": f"monthly observation; price precision=10 EUR; source_id={sid}",
                            }
                        )
                        existing.add(key)
                        added += 1
                ...
                failures += 1
                print(f"WARNING: {sid} {seed}: {e}")
            time.sleep(0.5)
        print(f"INFO: {sid} {name}: extracted={se} classified_new={sa}")
    save_obs(rows)
    build_outputs(rows)
    print(
        f"LEONES price collector: +{added} observations; extracted={extracted}; sources_ok={len(successful)}/{len(targets)}; source_attempt_failures={failures}; history={len(rows)}"
    )
    if not rows:
        raise SystemExit("No valid price observations exist after collection.")
