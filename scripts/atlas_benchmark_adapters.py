...
SOURCES = {
    "swe_bench": (
        ...
        "data/prospection/swe_bench_observations.csv",
    ),
    "livecodebench": (
        "https://livecodebench.github.io/",
        "data/prospection/livecodebench_observations.csv",
    ),
}
FIELDS = [
    "model_id",
...
def main():
    ...
    for source, (url, fn) in SOURCES.items():
        p = ROOT / fn
        if not p.exists():
            print(f"No structured {source} input yet: {p}")
            continue
        with p.open(encoding="utf-8", newline="") as f:
            rows = csv.DictReader(f)
        for r in rows:
            model = r.get("model_name", "")
            benchmark = r.get("benchmark") or source
            metric = r.get("metric", "")
            value = r.get("value", "")
            unit = r.get("unit", "")
            ...
            existing.append(
                {
                    ...
                    "retrieved_at": r.get("retrieved_at") or now,
                    "claim": claim,
                    "metric": metric,
                    "value": value,
                    "unit": unit,
                    ...
                    "workload": r.get("workload", "coding/agent"),
    ...
    print(
        f"Coding benchmarks: {added} new observations; {len(existing)} total evidence records"
    )
