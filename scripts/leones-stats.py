...
def load_results(root: Path, exclude_demo: bool) -> list[dict]:
    ...
    for path in root.rglob("*.json"):
        ...
        if (
            ...
            or data.get("status") not in VALID_STATUSES
        ):
            continue
...
def aggregate(results: list[dict]) -> dict:
    ...
    ram = Counter()
    speeds = []
    lotb = defaultdict(Counter)
    for result in official:
        hw = result.get("hardware", {})
        if hw.get("profile"):
            profiles[str(hw["profile"])] += 1
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Agrega resultados LEONES sin convertirlos en verificaciones"
    )
    p.add_argument("--root", default="results")
    p.add_argument("--output", default="web/data/stats.json")
    p.add_argument("--exclude-demo", action="store_true")
    ...
    results = load_results(Path(a.root), a.exclude_demo)
