...
One job: statistics. It consumes structured results and produces structured
statistics. It does not benchmark, publish, or generate individual reports.
"""
...
def aggregate(results:list[dict])->dict:
    ...
    return {
        "schema_version":"1.0","result_count":len(results),"official_count":len(official),"verified_count":len(verified),
        "status_counts":dict(statuses),"hardware_profiles":dict(profiles),"ram_gb":dict(ram),
        "generation_tokens_per_second":{"count":len(speeds),"min":min(speeds) if speeds else None,"max":max(speeds) if speeds else None,
        "average":round(sum(speeds)/len(speeds),3) if speeds else None,"at_least_10":sum(x>=10 for x in speeds),"at_least_100":sum(x>=100 for x in speeds)},
        "lotb":{code:dict(counts) for code,counts in sorted(lotb.items())}
    }

def main()->int:
    p=argparse.ArgumentParser(description="Aggregate LEONES result JSON files")
    p.add_argument("--root",default="results"); p.add_argument("--output",default="web/data/stats.json"); p.add_argument("--exclude-demo",action="store_true")
    a=p.parse_args(); output=aggregate(load_results(Path(a.root),a.exclude_demo)); destination=Path(a.output); destination.parent.mkdir(parents=True,exist_ok=True); destination.write_text(json.dumps(output,indent=2)+"\n",encoding="utf-8"); print(json.dumps(output,indent=2)); return 0
