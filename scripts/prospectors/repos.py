...
kind = sys.argv[1]
queries = {
    "runtimes": "local LLM inference runtime",
    "agents": "LLM agent framework local AI",
    "skills": "LLM agent skills",
    "harnesses": "LLM evaluation harness benchmark",
}
...
req = urllib.request.Request(
    url,
    headers={
        "User-Agent": "LEONES-Atlas-Prospector/1.0",
...
out = {
    ...
    "kind": kind,
    "policy": "discover-only; verify OSI license before recommendation",
    "items": [],
    "error": error or None,
}
for x in items:
...
if error:
    print(
        f"WARNING: GitHub discovery unavailable for {kind}: {error}; wrote empty/partial discovery output and continued."
    )
else:
