...
COMPLETE_SYSTEM = re.compile(
    r"\b(?:pc|ordenador|computador|desktop|port[aá]til|laptop|notebook|barebone|all[ -]?in[ -]?one)\b",
    re.I,
)
...
def assess(row):
    ...
    if COMPLETE_SYSTEM.search(name):
        score -= 80
        reasons.append("complete_system_or_laptop")
    if kind == "cpu" and not CPU_WORDS.search(name):
        score -= 60
        reasons.append("cpu_model_not_detected")
    if kind == "gpu" and (
        not GPU_WORDS.search(name) or "nvidia" not in (row.get("vendor") or "").lower()
    ):
        score -= 60
        reasons.append("gpu_model_not_detected")
    if kind == "ram" and not RAM_WORDS.search(name):
        score -= 60
        reasons.append("ram_model_not_detected")
    try:
...
def main():
            ...
            w.writerow(
                {
                    ...
                    "model": r.get("model", ""),
                    "capacity_gb": r.get("capacity_gb", ""),
                    "price_eur": r.get("price_eur", ""),
                    "price_type": "observed",
                    "market": r.get("market", "Spain"),
            ...
            w.writerow(
                dict(
                    zip(
                        ...
                        [
                            *key,
                            len(set(v.get("source", "") for v in values)),
                            ";".join(sorted(set(v.get("source", "") for v in values))),
                            f"{min(prices):.2f}",
                            f"{median(prices):.2f}",
                            f"{max(prices):.2f}",
                            max(v.get("observed_at", "") for v in values),
                        ],
                    )
                )
            )
    ...
    if rejected:
        print(
            "QUALITY: rejected observations remain in the immutable monthly ledger; they are excluded from summaries/recommender input."
        )
