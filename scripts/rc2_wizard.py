...
STACKS = {
    "ODS": {"name":"ods","adapter":"ods.v1","mode":"local-stack","title_key":"ods_title","summary_key":"ods_summary","next_step_key":"next_step_ods","install_script":"scripts/integrations/install_ods.sh","capability_keys":tuple(f"ods_capability_{i}" for i in range(1,5))},
    "Magnitude": {"name":"magnitude","adapter":"magnitude.v1","mode":"agent","title_key":"magnitude_title","summary_key":"magnitude_summary","next_step_key":"next_step_magnitude","install_script":"scripts/integrations/install_magnitude.sh","capability_keys":tuple(f"magnitude_capability_{i}" for i in range(1,5))},
}
A01_BENCHMARK={"id":"LEONES-Agentic","version":"1.0","task":"A01","task_version":"1.0","prompt":"Execute A01. Return only JSONL tool calls.","metrics":["wall_seconds","measured_tps","grader_pass"]}
...
def _show_multiline(io:WizardIO,text:str,prefix:str="")->None:
    for line in text.splitlines(): io.show(f"{prefix}{line}")

def _choose(io:WizardIO,title:str,options:tuple[str,...])->str:
    io.show(""); _show_multiline(io,title); io.show("┌──────────────────────────────────────────────────────────┐")
    for i,option in enumerate(options,1):
        lines=option.splitlines(); io.show(f"│  [{i}] {lines[0]}")
        for line in lines[1:]: io.show(f"│      {line}")
    io.show("└──────────────────────────────────────────────────────────┘")
    while True:
...
def _choose_language(io:WizardIO)->str:
    labels=tuple(LANGUAGE_LABELS[code] for code in LANGUAGES); io.show(""); io.show("ELIGE EL IDIOMA / CHOOSE LANGUAGE / 选择语言"); io.show("┌──────────────────────────────────────────────────────────┐")
    for i,label in enumerate(labels,1): io.show(f"│  [{i}] {label}")
    io.show("└──────────────────────────────────────────────────────────┘")
    while True:
        answer=io.ask("LEONES> ").strip()
        if answer.isdigit() and 1<=int(answer)<=len(LANGUAGES): return set_language(LANGUAGES[int(answer)-1])
        io.show("  ! Invalid option / Opción no válida / 无效选项")

def _live_inputs(io:WizardIO)->tuple[dict[str,Any],list[dict[str,Any]]]:
    io.show(""); _show_multiline(io,tr("detecting_hardware"),prefix="[INFO] ")
    try: system=run_system(); result=run_recommend(limit=5)
    except LLMFitError as exc: io.show(f"[!] {tr('llmfit_unavailable')}: {exc}"); raise
    hardware=normalize_hardware(normalise_hardware(system)); raw_candidates=normalise_candidates(result)
...
def _show_stack(io:WizardIO,name:str)->None:
    stack=STACKS[name]; io.show(""); _show_multiline(io,tr(stack["title_key"])); _show_multiline(io,tr(stack["summary_key"]),prefix="  → ")
    for capability_key in stack["capability_keys"]: _show_multiline(io,tr(capability_key),prefix="  ✓ ")

def _stack_choice_labels()->tuple[str,...]: return tuple(tr(STACKS[name]["summary_key"]) for name in STACKS)

def _show_decision_summary(io:WizardIO,*,model_name:str,stack_name:str,stack:dict[str,Any])->None:
    script=stack["install_script"]; io.show(""); io.show("═"*60); _show_multiline(io,tr("what_was_decided")); io.show("═"*60); io.show(f"  {tr('label_model')}: {model_name}"); io.show(f"  {tr('label_stack')}: {stack_name} ({stack['name']})"); io.show(f"  {tr('label_status')}: {tr('status_authorized_not_installed')}"); io.show(""); _show_multiline(io,tr("installation_authorized"),prefix="[✓] "); _show_multiline(io,tr("not_installed_yet"),prefix="[!] "); io.show(""); _show_multiline(io,tr("next_step_title")); _show_multiline(io,tr(stack["next_step_key"]),prefix="  "); io.show(f"  $ bash {script}"); io.show(""); _show_multiline(io,tr("next_step_after_install"),prefix="[i] ")

def _maybe_run_installer(io:WizardIO,stack:dict[str,Any])->dict[str,Any]:
    script=REPO_ROOT/stack["install_script"]; choice=_choose(io,tr("offer_run_installer"),(tr("run_installer_yes"),tr("run_installer_no")))
    if choice==tr("run_installer_no"): _show_multiline(io,tr("installer_deferred"),prefix="[i] "); return {"status":"deferred","script":str(script.relative_to(REPO_ROOT)),"real_installation":False}
    _show_multiline(io,tr("installer_launching"),prefix="[INFO] "); io.show(f"[INFO] $ bash {script.relative_to(REPO_ROOT)}")
    try: rc=subprocess.run(["bash",str(script)],cwd=str(REPO_ROOT),check=False).returncode
    except OSError as exc: io.show(f"[!] {exc}"); rc=1
    if rc==0: _show_multiline(io,tr("installer_finished_ok"),prefix="[✓] "); return {"status":"installer_exited_0","script":str(script.relative_to(REPO_ROOT)),"returncode":rc,"real_installation":False}
    _show_multiline(io,tr("installer_finished_fail"),prefix="[!] "); return {"status":"installer_failed_or_cancelled","script":str(script.relative_to(REPO_ROOT)),"returncode":rc,"real_installation":False}
...
def _show_verification(io:WizardIO,result:dict[str,Any])->None:
    ...
    if result.get("status")=="PASS" and result.get("real_installation") is True: _show_multiline(io,tr("verify_pass"),prefix="[✓] ")
    else: _show_multiline(io,tr("verify_fail"),prefix="[!] ")
...
def _run_physical_verification(io:WizardIO,stack_name:str)->dict[str,Any]:
    while True:
        io.show(""); _show_multiline(io,tr("verify_running"),prefix="[INFO] "); result=verify_stack(stack_name).to_dict(); _show_verification(io,result)
        if result.get("status")=="PASS" and result.get("real_installation") is True: _show_multiline(io,tr("verify_next_pass"),prefix="[✓] "); return result
        _show_multiline(io,tr("verify_next_fail"),prefix="[!] "); again=_choose(io,tr("offer_verify_again"),(tr("verify_again_yes"),tr("verify_again_no")))
...
def _run_a01(io:WizardIO,*,model_choice:dict[str,Any],hardware:dict[str,Any],resolution:dict[str,Any])->dict[str,Any]:
    ...
    if rc!=0 or not out_path.exists(): _show_multiline(io,tr("benchmark_failed"),prefix="[!] "); return {"status":"benchmark_failed","returncode":rc,"out":str(out_path.relative_to(REPO_ROOT)),"measured":False,"resolution":resolution}
    try: payload=json.loads(out_path.read_text(encoding="utf-8"))
    except (OSError,json.JSONDecodeError) as exc: _show_multiline(io,tr("benchmark_failed"),prefix="[!] "); return {"status":"benchmark_failed","reason":str(exc),"measured":False,"resolution":resolution}
    evidence=payload.get("evidence") or {}; rb=evidence.get("runtime_benchmark") or {}; execution_id=rb.get("execution_id"); measured=rb.get("measurement_status")=="measured"; io.show(""); _show_multiline(io,tr("benchmark_completed"),prefix="[✓] ");
    if execution_id: io.show(f"  execution_id: {execution_id}")
    if rb.get("wall_seconds") is not None: io.show(f"  wall_seconds: {rb.get('wall_seconds')}")
    if rb.get("measured_tps") is not None: io.show(f"  measured_tps: {rb.get('measured_tps')}")
    if rb.get("grader_pass") is not None: io.show(f"  grader_pass: {rb.get('grader_pass')}")
    io.show(f"  evidence: {out_path.relative_to(REPO_ROOT)}")
    return {"status":"benchmark_completed" if measured else "benchmark_failed","returncode":rc,"out":str(out_path.relative_to(REPO_ROOT)),"execution_id":execution_id,"runtime_benchmark":rb,"measured":measured,"resolution":resolution}
...
def run_wizard(io:WizardIO|None=None)->BetaSession:
    io=io or WizardIO(); session=BetaSession(); io.show(BANNER); language=_choose_language(io); session.data["ui_language"]=language; _show_multiline(io,tr("banner_subtitle")); _show_multiline(io,tr("your_team")); io.show("")
    ...
    session.advance("HARDWARE_READY",hardware=hardware); _show_multiline(io,tr("hardware_ready"),prefix="[✓] "); _show_multiline(io,tr("estimated_notice"),prefix="[i] "); labels=tuple(f"{c['name']} · fit={c['fit']} · ~{c['estimated_tps']} tok/s · {c['source']} · ESTIMATED" for c in candidates)
    ...
    plan=to_selection_plan(chosen,hardware,{"name":stack["name"],"adapter":stack["adapter"],"mode":stack["mode"]}); session.advance("STACK_SELECTED",stack=stack,stack_selection=stack,selection_plan=plan); _show_multiline(io,tr("selected"),prefix=f"[✓] {stack_name} / "); session.advance("CONSENT_REQUIRED",installation={"status":"plan_ready"}); install=_choose(io,tr("install_consent"),(tr("authorize"),tr("cancel")))
    if install==tr("cancel"): session.block("INSTALL_DECLINED",tr("install_blocked")); return session
    session.authorize_installation(); model_name=str(chosen.get("name") or chosen.get("model_id") or "unknown"); _show_decision_summary(io,model_name=model_name,stack_name=stack_name,stack=stack); install_result=_maybe_run_installer(io,stack); session.data["installation"]={"status":"INSTALLING","consent":"granted","stack":stack["name"],"installer":install_result}; verification=_run_physical_verification(io,stack["name"]); session.data["installation"]["verification"]=verification
    if verification.get("real_installation") is not True: session.block("PHYSICAL_VERIFY_FAILED",verification.get("message") or tr("verify_fail")); return session
    session.installation_verified({"status":verification.get("status"),"real_installation":True,"stack":stack["name"],"checks":verification.get("checks"),"observed":verification.get("observed"),"message":verification.get("message")}); _benchmark_gate(io,session,model_choice=chosen,hardware=hardware); return session
