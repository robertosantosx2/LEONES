...
def run_wizard(io:WizardIO|None=None)->BetaSession:
    ...
    plan=to_selection_plan(chosen,hardware,{"name":stack["name"],"adapter":stack["adapter"],"mode":stack["mode"]}); session.advance("STACK_SELECTED",stack_selection=stack,selection_plan=plan); io.show(f"[✓] {stack_name} / selected / 已选择")
