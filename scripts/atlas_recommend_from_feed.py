    ...
    *legacy_args,
    ...
    if legacy_args:
        ...
        if len(legacy_args) == 6:
            ...
            raise TypeError(
                "legacy recommend signature requires 6 or 7 positional arguments"
            )
        result = select(
            rows,
            workload=workload,
        ...
        return [
            (
                ...
                _legacy_class(
                    source_by_id.get(item.get("model_id"), {}).get("tokens_per_second")
                ),
            )
...
def write_legacy_csv(
    result: dict, source_rows: list[dict[str, str]], output: Path
) -> None:
...
def main() -> int:
    ...
    print(
        f"eligible={result['counts']['eligible']} top_n={result['counts']['top_n']} rejected={result['counts']['rejected']} -> {args.out}"
    )
    return 0
