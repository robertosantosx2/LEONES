...
1. eligibility removes models that cannot be justified for the requested task/hardware;
2. LLMFit is an optional pre-filter/estimate and is never a measurement;
3. ranking orders eligible candidates without using hardware price;
...
def eligibility(row: dict[str, Any], *, workload: str, hardware: str, ram_gb: float,
    ...
    if match:
        ...
        evidence["llmfit"] = {"fit_level": fit, "estimated_tps": estimated_tps,
...
def score(row: dict[str, Any], evidence: dict[str, Any]) -> tuple[float, list[str]]:
    ...
    reasons = [f"quality={quality if quality is not None else 'unknown'}",
...
def select(rows: Iterable[dict[str, Any]], *, workload: str, hardware: str, ram_gb: float,
           ...
           llmfit: dict[str, Any] | None = None, require_llmfit_fit: bool = False) -> dict[str, Any]:
    """Produce the canonical, deterministic selection result."""
    rows = list(rows)
    selected: list[dict[str, Any]] = []
    rejected: list[dict[str, Any]] = []
    for row in rows:
        ok, reasons, evidence = eligibility(row, workload=workload, hardware=hardware,
            ram_gb=ram_gb, vram_gb=vram_gb, context_tokens=context_tokens,
            llmfit=llmfit, require_llmfit_fit=require_llmfit_fit)
        if not ok:
            rejected.append({"model_id": row.get("model_id") or row.get("model_name"),
        ...
        selected.append({"model_id": row.get("model_id") or row.get("model_name"),
            ...
            "runtime": row.get("runtime"), "selection_status": "CANDIDATE",
            "task_fit": "compatible", "hardware_fit": "compatible", "memory_fit": "compatible",
    ...
    return {"schema_version": "1.0", "selector": "LEONES-model-selection",
        "selection_policy": {"workload": workload, "hardware": hardware, "ram_gb": ram_gb,
            "vram_gb": vram_gb, "context_tokens": context_tokens, "top_n": top_n,
            "price_in_score": False, "measured_performance_required_for_final_claim": True},
        "counts": {"input": len(rows), "eligible": len(selected), "rejected": len(rejected), "top_n": len(top)},
        "candidates": selected, "rejected": rejected}
...
def main() -> int:
    ...
    result = select(rows, workload=args.workload, hardware=args.hardware, ram_gb=args.ram,
        vram_gb=args.vram, context_tokens=args.context, top_n=args.top_n, llmfit=llmfit,
