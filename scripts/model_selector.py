...
def _runtime_evidence(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "model": {
            ...
            "quantized_weight_gb": _num(
                row.get("quantized_weight_gb") or row.get("weight_memory_gb")
            ),
        },
...
def eligibility(
    ...
    *,
    ...
    if level not in {"T2", "T3"}:
        return (
            False,
            [f"technical evidence level {level or 'unknown'} is below T2"],
            evidence,
        )
    ...
    if required_runtime and _norm(runtime) != _norm(required_runtime):
        return (
            False,
            [f"runtime mismatch: {runtime!r} != {required_runtime!r}"],
            evidence,
        )
    ...
    declared_opts = [
        item.strip()
        for item in str(row.get("optimization_families") or "").split(",")
        if item.strip()
    ]
    ...
    missing_opts = [
        item for item in required_opts if _norm(item) not in declared_normalized
    ]
    if missing_opts:
        return (
            False,
            [f"candidate lacks selected optimization families: {missing_opts}"],
            evidence,
        )
    ...
    if memory > available:
        return (
            False,
            [f"memory requirement {memory:g} GB exceeds available {available:g} GB"],
            evidence,
        )
    ...
    evidence["context_supported"] = (
        int(supported_context) if supported_context is not None else None
    )
    evidence["context_target"] = context_tokens
    evidence["context_recommended"] = (
        min(int(supported_context), context_tokens)
        if supported_context is not None
        else None
    )
    ...
    if match:
        ...
        estimated_tps = _num(
            match.get("estimated_tps") or match.get("tok_s") or match.get("tps")
        )
        evidence["llmfit"] = {
            "fit_level": fit,
            "estimated_tps": estimated_tps,
            "memory_required_gb": _num(
                match.get("memory_required_gb") or match.get("memory_gb")
            ),
        ...
        selected.append(
            {
                ...
                "parameter_selection_basis": runtime_evidence[
                ...
                if evidence.get("context_supported") is not None
                else "unknown",
                ...
                "fit_score": fit_score,
                "confidence": "high"
                if row.get("quality_score") and row.get("tokens_per_second")
                else "medium",
...
def main() -> int:
    ...
    llmfit = (
        json.loads(args.llmfit.read_text(encoding="utf-8")) if args.llmfit else None
    )
    result = select(
        rows,
        ...
        optimization_families=args.optimization,
    )
    ...
    print(
        f"selected={result['counts']['eligible']} top_n={result['counts']['top_n']} rejected={result['counts']['rejected']} -> {args.out}"
    )
    return 0
