...
def registry_entries(path: Path = REGISTRY_PATH) -> dict[str, RuntimeEntry]:
    ...
    required = {
        ...
        "deployment_class", "serving_profiles",
    }
    for raw in data["runtimes"]:
        ...
        entry = RuntimeEntry(
            ...
            version=raw["version"],
            aliases=aliases,
            deployment_class=_as_strings(raw["deployment_class"], "deployment_class", runtime_id),
            serving_profiles=_as_strings(raw["serving_profiles"], "serving_profiles", runtime_id),
