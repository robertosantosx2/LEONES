...
FIELDS=['source_file','source_id','model_id','model_name','organization','release_date','source_url','repository_url','repository_host','license','weights_url','code_url','runtime','format','quantization','hardware_id','workload','jgb_level','jgb_confidence','quality_score','tokens_per_second','estimated_memory_gb','context_tokens','evidence_status','notes']
FILES=['classified_discoveries.ndjson','additional_forge_discoveries.ndjson']
ALIASES={'id':'source_id','model':'model_id','model_id':'model_id','name':'model_name','repo':'model_id','repository':'model_id','repository_id':'model_id','repo_name':'model_id','url':'source_url','evidence_url':'source_url','repository_url':'repository_url','repo_url':'repository_url','date':'release_date','license_name':'license','weights':'weights_url','code':'code_url'}
FORGES={'github.com':'https://github.com/','gitlab.com':'https://gitlab.com/','codeberg.org':'https://codeberg.org/','gitea.com':'https://gitea.com/','framagit.org':'https://framagit.org/','pagure.io':'https://pagure.io/','git.zx2c4.com':'https://git.zx2c4.com/','huggingface.co':'https://huggingface.co/'}
...
def canonical_repo(base_url, model_id):
    ...
    if not re.fullmatch(r'[^/\s]+/[^/\s]+', path): return ''
    return prefix + path
...
def normalize(obj, source):
    ...
    if r['source_url'] and host_of(r['source_url']) in FORGES and r['repository_url']:
        r['notes']=(r['notes']+'; canonical repository URL derived from forge + repository path').strip('; ')
    return r
...
def main():
    ...
    unique={}
    for r in records:
        key=r['repository_url'] or r['source_id'] or f"{r['model_id']}|{r['source_url']}"
    ...
    print(f'Atlas ingest: {len(unique)} model records; {repos} canonical repositories; {len(review)} require verification')
