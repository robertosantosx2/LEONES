...
SOURCES = {
    "vellum": (
        ...
        "data/prospection/vellum_observations.csv",
    ),
    "lambda": (
        "https://lambda.ai/llm-benchmarks-leaderboard",
        "data/prospection/lambda_observations.csv",
    ),
}
FIELDS = [
    "model_id",
...
def main():
    ...
    for source, (url, fn) in SOURCES.items():
        ...
        for r in rows:
            ...
            key = "|".join(
                [source, model, benchmark, metric, value, r.get("setting", "")]
            )
            ...
            existing.append(
                {
                    ...
                    "workload": r.get("workload", "benchmark"),
    ...
    print(
        f"Vellum/Lambda: {added} new observations; {len(existing)} total evidence records"
    )
