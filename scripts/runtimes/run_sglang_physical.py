#!/usr/bin/env python3
"""Prepare/execute the SGLang CPD physical execution boundary."""
...
def build_command(model: str, host: str, port: int, *, tp_size: int = 1) -> list[str]:
    return [
...
def main() -> int:
    ...
    command = build_command(args.model, args.host, args.port, tp_size=args.tp_size)
    ...
    if not args.execute:
        return 0
    raise SystemExit("Physical SGLang execution is intentionally gated for the CPD host stage.")
