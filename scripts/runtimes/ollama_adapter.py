...
class OllamaAdapter(RuntimeAdapter):
    ...
    def prepare(
        ...
        return RuntimeExecutionSpec(
            ...
            {"protocol": "ollama-api", "metrics": entry.metrics, "runner": "ollama"},
        )
...
def prepare(plan: Any) -> RuntimeExecutionSpec:
    if getattr(plan, "runtime_id", None) != "ollama":
        raise ValueError(
            f"unsupported runtime for Ollama adapter: {getattr(plan, 'runtime_id', None)!r}"
        )
    if getattr(plan, "adapter_id", None) != ADAPTER_ID:
        raise ValueError(
            f"unsupported adapter for Ollama: {getattr(plan, 'adapter_id', None)!r}"
        )
    return RuntimeExecutionSpec(
        "ollama",
        ADAPTER_ID,
        plan.model_ref,
        ("ollama",),
        {"protocol": "ollama-api", "metrics": "api-defined", "runner": "ollama"},
    )
