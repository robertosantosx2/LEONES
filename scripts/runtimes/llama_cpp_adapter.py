...
def build_command(executable: str, model_path: str, prompt: str, *, context_tokens: int | None = None) -> list[str]:
    ...
    if context_tokens is not None:
        if context_tokens < 1: raise ValueError("context_tokens must be positive")
        command.extend(["-c", str(context_tokens)])
    return command

def build_command_from_plan(plan: dict[str, Any], model_path: str, prompt: str, *, executable: str = "llama-cli",
                            context_tokens: int | None = None) -> list[str]:
    ...
    runtime_id = runtime_value.get("name") if isinstance(runtime_value, dict) else runtime_value
    if runtime_id != "llama.cpp": raise ValueError("unsupported runtime for llama.cpp adapter")
    if executable != "llama-cli": raise ValueError("executable is not the trusted llama.cpp registry entrypoint")
