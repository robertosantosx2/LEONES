...
def canonical(item: dict) -> str:
    url = (
        (item.get("url") or item.get("evidence_url") or "").strip().rstrip("/").lower()
    )
    if url:
        return url
...
def merge_item(existing: dict, incoming: dict) -> dict:
    ...
    if incoming_record.get("evidence_url") not in {
        r.get("evidence_url") for r in records
    }:
...
def main():
    ...
    p.add_argument(
        "--federated", default="data/prospection/federated_discoveries.ndjson"
    )
    ...
    report = {
        ...
        "multi_source_records": sum(
            1 for x in merged.values() if x.get("source_count", 0) > 1
        ),
    ...
    (output.parent / "discovery_merge_report.json").write_text(
