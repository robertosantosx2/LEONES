...
def row(source, adapter, base, name, url, query, kind="software", description=""):
    return {
        ...
        "provenance": {"adapter": adapter, "source_id": source},
    }


def onedev(source, base, q):
    urls = [
        base + "/projects?query=" + urllib.parse.quote(q),
        base + "/projects?search=" + urllib.parse.quote(q),
    ]
    for u in urls:
        try:
            html, status = get(u)
...
def fossil(source, base, q):
    for u in [
        base + "/search?search=" + urllib.parse.quote(q),
        base + "/search?searchterm=" + urllib.parse.quote(q),
    ]:
        try:
            html, status = get(u)
            p = Links()
    ...
    try:
        ...
        for href, label in p.links:
            ...
            if href.startswith("/") and href.count("/") == 2:
                ...
                out.append(
                    row(
                        source, "cgit", base, label, urllib.parse.urljoin(base, href), q
                    )
                )
                if len(out) >= 20:
                    break
    ...
    for u in [base + "/source/", base + "/maniphest/"]:
        try:
            ...
            for href, label in p.links:
                if not label or href in seen:
                    continue
                if needle in (label + " " + href).lower() and (
                    "/source/" in href or "/maniphest/" in href
                ):
                    seen.add(href)
                    out.append(
                        row(
...
def main():
    out = Path("data/prospection/additional_forge_discoveries.ndjson")
    report = Path("data/prospection/additional_forge_report.json")
    fed = Path("data/prospection/federated_discoveries.ndjson")
    ...
    if fed.exists():
        for line in fed.read_text(encoding="utf-8").splitlines():
            try:
                x = json.loads(line)
                existing.add(x.get("url") or f"{x.get('source')}:{x.get('name')}")
            except Exception:
                pass
    ...
    data = {
        ...
        "merged_into_federation": sum(
            1
            for r in unique.values()
            if (r["url"] or f"{r['source']}:{r['name']}") in existing
        ),
        "errors": errors,
        "active_adapters": sorted(TARGETS),
        "note": "Only concrete public instances are queried; platform websites without repository instances remain outside this adapter.",
    }
    report.write_text(
