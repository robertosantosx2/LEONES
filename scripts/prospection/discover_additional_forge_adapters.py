...
UA = "LEONES-Atlas-Prospection/2.7"
...
def onedev(source, base, q):
    # Public OneDev instance. Its project search is exposed in the web UI.
    ...
    for u in urls:
        try:
            ...
            for href,label in p.links:
                ...
                if "/" in href and (href.startswith("/onedev/") or "/projects/" in href):
...
def fossil(source, base, q):
    # Fossil's built-in web UI has full-text search. Use the public repository
    # instances and retain only repository/project-looking links.
...
def cgit(source, base, q):
    # cgit supports q= for repository/global search. The concrete instance is
    # the public git.zx2c4.com deployment.
...
def phorge(source, base, q):
    # Public Phorge instance. Prefer the public project/maniphest search pages;
    # if authentication is required, report zero rather than fabricate results.
...
def main():
    out=Path("data/prospection/additional_forge_discoveries.ndjson"); report=Path("data/prospection/additional_forge_report.json"); out.parent.mkdir(parents=True,exist_ok=True)
    ...
    data={"generated_at":datetime.now(timezone.utc).isoformat(),"sources":len(TARGETS),"raw_results_by_source":dict(stats),"unique_discoveries":len(unique),"errors":errors,"active_adapters":sorted(TARGETS),"note":"Only concrete public instances are queried; platform websites without repository instances remain outside this adapter."}
