...
def main():
    ap = argparse.ArgumentParser(
        description="Normalize model discoveries for LEONES Atlas"
    )
    ...
    if src.exists():
        for line in src.read_text(encoding="utf-8").splitlines():
            ...
            raw = json.loads(line)
            x = {
                k: (norm(raw.get(k)) if k in FIELDS else raw[k])
                for k in raw
                if k in FIELDS
            }
            if not x.get("name"):
            ...
            x.update(
                {
                    "type": "model",
                    "discovery_id": raw.get("discovery_id") or discovery_id(x),
    ...
    print(
        json.dumps(
            {"bot": "models", "discovered": len(rows), "output": str(out)},
            ensure_ascii=False,
        )
    )
