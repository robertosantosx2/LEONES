...
def get(url, source_id=None, adapter=None):
    req=urllib.request.Request(url,headers={"User-Agent":UA,"Accept":"application/json, text/html;q=0.9, */*;q=0.8"})
    token_env={"gitlab":{"gitlab":"LEONES_GITLAB_TOKEN","framagit":"LEONES_FRAMAGIT_TOKEN"},"huggingface-models":{"huggingface":"HF_TOKEN"},"huggingface-papers":{"huggingface-papers":"HF_TOKEN"}}.get(adapter or "",{}).get(source_id or "")
...
def search_gitlab(s,b,q):
    raw,status=get(b.rstrip("/")+"/api/v4/projects?"+urllib.parse.urlencode({"search":q,"order_by":"last_activity_at","sort":"desc","per_page":20}),s,"gitlab"); data=json.loads(raw); now=datetime.now(timezone.utc).isoformat(); return [normalize(s,"gitlab",b,x,q,now) for x in data],status
def search_forgejo(s,b,q):
    raw,status=get(b.rstrip("/")+"/api/v1/repos/search?"+urllib.parse.urlencode({"q":q,"limit":20}),s,"forgejo"); data=json.loads(raw); now=datetime.now(timezone.utc).isoformat(); items=data.get("data",[]) if isinstance(data,dict) else []; return [normalize(s,"forgejo",b,x,q,now) for x in items],status
def search_pagure(s,b,q):
    raw,status=get(b.rstrip("/")+"/api/0/projects?"+urllib.parse.urlencode({"pattern":q}),s,"pagure"); data=json.loads(raw); now=datetime.now(timezone.utc).isoformat(); items=data if isinstance(data,list) else data.get("projects",[]); return [normalize(s,"pagure",b,x,q,now) for x in items[:20]],status
def search_huggingface_models(s,b,q):
    raw,status=get(b+"/api/models?"+urllib.parse.urlencode({"search":q,"limit":20,"sort":"lastModified","direction":-1}),s,"huggingface-models"); data=json.loads(raw); now=datetime.now(timezone.utc).isoformat(); rows=[]
...
def search_huggingface_papers(s,b,q):
    raw,status=get(b+"/api/daily_papers?"+urllib.parse.urlencode({"limit":100,"offset":0}),s,"huggingface-papers"); data=json.loads(raw); now=datetime.now(timezone.utc).isoformat(); items=data.get("recentPapers",data.get("papers",[])) if isinstance(data,dict) else data; tokens=[t.lower() for t in q.split() if t]; rows=[]
    for x in items or []:
...
class SavannahParser(HTMLParser):
    ...
    def handle_endtag(self,tag):
        if tag=="a" and self.capture:
            label=" ".join("".join(self.text).split()); self.links.append((self.href,label)); self.capture=False
