...
def get(url, source_id=None, adapter=None, timeout=15):
    ...
    token_env = {"gitlab": {"gitlab": "LEONES_GITLAB_TOKEN", "framagit": "LEONES_FRAMAGIT_TOKEN"}, "huggingface-models": {"huggingface": "HF_TOKEN"}, "huggingface-papers": {"huggingface-papers": "HF_TOKEN"}, "sourcehut": {"sourcehut": "SOURCEHUT_TOKEN", "srht": "SOURCEHUT_TOKEN"}}.get(adapter or "", {}).get(source_id or "")
    token = os.getenv(token_env) if token_env else None
    if token and adapter == "gitlab": req.add_header("PRIVATE-TOKEN", token)
    elif token and adapter == "sourcehut": req.add_header("Authorization", f"Bearer {token}")
...
def search_forgejo(s, b, q):
    raw, status = get(b.rstrip("/") + "/api/v1/repos/search?" + urllib.parse.urlencode({"q": q, "limit": 20, "includeDesc": "true"}), s, "forgejo")
...
def search_pagure(s, b, q):
    ...
    urls = [
        ...
        b.rstrip("/") + "/api/0/projects?" + urllib.parse.urlencode({"pattern": f"*{q}*"}),
    ]
    last_status = 200
    for url in urls:
        try:
            raw, status = get(url, s, "pagure"); last_status = status; data = json.loads(raw)
    ...
    html_url = b.rstrip("/") + "/search?" + urllib.parse.urlencode({"search_pattern": q})
    try:
        raw, status = get(html_url, s, "pagure"); last_status = status
        parser = LinkParser(patterns=("/",))
        parser.feed(raw)
        seen = set()
        for href, label in parser.links:
            if not label or href in seen or href.startswith(("/api/", "/issues", "/pull-request")): continue
            if href.count("/") >= 2 and not href.startswith("//"):
                seen.add(href); rows.append(normalize(s, "pagure", b, {"id": label, "url": urllib.parse.urljoin(b, href)}, q, now))
...
def search_gogs(s, b, q):
    ...
    api = b.rstrip("/") + "/api/v1/repos/search?" + urllib.parse.urlencode({"q": q, "limit": 20})
    try:
        raw, status = get(api, s, "gogs"); data = json.loads(raw); items = data.get("data", []) if isinstance(data, dict) else data
        if isinstance(items, list): return [normalize(s, "gogs", b, x, q, now) for x in items[:20]], status
    except Exception:
        pass
    web = b.rstrip("/") + "/explore/repos?" + urllib.parse.urlencode({"q": q})
    raw, status = get(web, s, "gogs"); parser = LinkParser(patterns=("/",)); parser.feed(raw); rows = []; seen = set()
    for href, label in parser.links:
        if not label or href in seen or not href.startswith("/") or href.startswith(("/explore", "/repo", "/user", "/api")): continue
        parts = href.strip("/").split("/")
        if len(parts) >= 2 and all(parts[:2]):
            seen.add(href); rows.append(normalize(s, "gogs", b, {"id": "/".join(parts[:2]), "url": urllib.parse.urljoin(b, "/".join(parts[:2]))}, q, now))
            if len(rows) >= 20: break
    return rows, status
...
def search_huggingface_papers(s, b, q):
    ...
    for x in items or []:
        ...
        if tokens and not any(t in blob for t in tokens): continue
...
def search_sourcehut(s, b, q):
    if not os.getenv("SOURCEHUT_TOKEN"): raise RuntimeError("SOURCEHUT_TOKEN is required for SourceHut search")
    raise RuntimeError("SourceHut adapter requires GraphQL endpoint validation")


class LinkParser(HTMLParser):
    def __init__(self, patterns=()): super().__init__(); self.links=[]; self.href=None; self.text=[]
...
def search_savannah(s, b, q):
    ...
    urls = [b.rstrip("/") + "/search/?" + urllib.parse.urlencode({"words": q}), b.rstrip("/") + "/search.php?" + urllib.parse.urlencode({"words": q})]
    for url in urls:
        try:
            raw, status = get(url, s, "savannah"); parser=LinkParser(); parser.feed(raw)
            for href,label in parser.links:
                if not label or href in seen: continue
                if "/projects/" in href or href.startswith("/project/"):
                    seen.add(href); rows.append(normalize(s,"savannah",b,{"id":label,"url":urllib.parse.urljoin(b,href)},q,now))
                    if len(rows)>=20: break
...
def main():
    ...
    fnmap={"gitlab":search_gitlab,"forgejo":search_forgejo,"gogs":search_gogs,"pagure":search_pagure,"huggingface-models":search_huggingface_models,"huggingface-papers":search_huggingface_papers,"savannah":search_savannah,"sourcehut":search_sourcehut,"gitbucket":lambda *a: (_ for _ in ()).throw(RuntimeError("GitBucket requires concrete instance discovery"))}
