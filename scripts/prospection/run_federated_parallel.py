...
def run_one(source: dict | None, queries: int, timeout: int, only_instances: bool = False):
    ...
    with tempfile.TemporaryDirectory(prefix="leones-source-") as td:
        ...
        registry.write_text(json.dumps({"version": "worker", "sources": [source] if source else []}, ensure_ascii=False), encoding="utf-8")
        ...
        try:
            ...
            return {"source_id": label, "returncode": proc.returncode, "rows": rows,
                    "report": report, "stdout": proc.stdout[-4000:], "stderr": proc.stderr[-4000:]}
        except subprocess.TimeoutExpired as exc:
            return {"source_id": label, "returncode": 124, "rows": [], "report": {},
...
def main():
    ...
    tasks = [(source, False) for source in sources] + [(None, True) for _ in ([instances] if instances else [])]
    results = []
    with ThreadPoolExecutor(max_workers=max(1, args.workers)) as pool:
        futures = [pool.submit(run_one, source, args.queries, args.timeout, only_instances) for source, only_instances in tasks]
