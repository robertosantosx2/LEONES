...
def main():
    ...
    parser.add_argument(
        "--registry", default=str(REGISTRY), help="Registro de fuentes."
    )
    parser.add_argument(
        "--plan", default=str(PLAN), help="Plan de familias y adaptadores."
    )
    parser.add_argument(
        "--output",
    ...
    for family in plan.get("families", []):
        for source_id in family.get("sources", []):
            ...
            for category, queries in CATEGORY_QUERIES.items():
                ...
                family_queries = list(
                    dict.fromkeys(family.get("queries", []) + queries)
                )
