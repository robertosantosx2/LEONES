...
EXTRA = [
    ...
    "technical_profile_level",
    "technical_evidence_checked_at",
]
...
def infer_quant(config, model_id):
    text = (
        " ".join(
            str(config.get(k, ""))
            for k in ("quantization_config", "quantization")
    ...
    try:
        ...
        for key in (
            ...
            if value and value < 10_000_000:
                out["context_tokens"] = str(int(value))
                break
        ...
        t3 = bool(
            out.get("tokens_per_second")
            and out.get("hardware_id")
            and out.get("runtime")
        )
        out["technical_profile_level"] = (
            "T3" if t3 else "T2" if t2 else "T1" if t1 else "T0"
        )
        out["technical_evidence_checked_at"] = time.strftime(
        ...
        out["notes"] = (
            out.get("notes", "")
            + f"; technical evidence unavailable: {type(exc).__name__}"
        ).strip("; ")
    return out
...
def main():
    ...
    for i, row in enumerate(enriched):
        for field in EXTRA:
            if row.get(field) and not rows[i].get(field):
    ...
    with FEED.open("w", encoding="utf-8", newline="") as f:
        ...
        w.writerows(rows)
    counts = {
        k: sum(r.get("technical_profile_level") == k for r in enriched)
        for k in ("T0", "T1", "T2", "T3")
    }
    reported = sum(r.get("technical_evidence_state") == "reported" for r in enriched)
    print(
        f"Technical evidence: reported={reported}/{len(enriched)}; T0={counts['T0']} T1={counts['T1']} T2={counts['T2']} T3={counts['T3']}; output={EVIDENCE}"
    )
