...
TASKS = {
    ...
    "coding": {
        "capabilities": ["code_generation", "code_reasoning", "tool_use"],
        "quality": "coding",
    },
    "rag": {
        "capabilities": ["long_context", "retrieval", "grounded_answer"],
        "quality": "knowledge",
    },
    "vision": {"capabilities": ["vision", "multimodal"], "quality": "multimodal"},
    "agent": {
        "capabilities": ["tool_use", "planning", "structured_output"],
        "quality": "agentic",
    },
}
...
def infer(text: str, kind: str | None):
    ...
    if any(
        ...
        return "coding"
    if any(x in t for x in ("imagen", "foto", "visual", "multimodal")):
        return "vision"
    if any(
        x in t
        for x in (
        ...
        return "rag"
    if any(
        x in t for x in ("agente", "agent", "herramientas", "automatizar una tarea")
    ):
        return "agent"
    return "chat"
...
def extract_constraints(text: str):
    ...
    latency = None
    m = re.search(
        r"(?:menos de|<|máximo|max)\s*(\d+(?:[.,]\d+)?)\s*(?:s|seg|segundos)", t
    )
    if m:
        latency = float(m.group(1).replace(",", "."))
    memory = None
    m = re.search(
        r"(?:menos de|<|máximo|max)\s*(\d+(?:[.,]\d+)?)\s*(?:gb|gib)\s*(?:de\s*)?(?:ram|memoria)?",
        t,
    )
    if m:
        memory = float(m.group(1).replace(",", "."))
    gpu = None
    if any(x in t for x in ("sin gpu", "cpu-only", "solo cpu", "sin gráfica")):
        gpu = False
    elif any(x in t for x in ("con gpu", "nvidia", "cuda", "gráfica")):
        gpu = True
    return {
        "local_only": True,
