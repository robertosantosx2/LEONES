...
def get_rows(offset: int = 0, length: int = 100):
    ...
    req = urllib.request.Request(
        f"{API}?{q}", headers={"User-Agent": "LEONES-Atlas/0.3"}
    )
            ...
            out.append(
                {
                    ...
                    "value": value,
                    "unit": "score",
                    "benchmark": metric.split("_")[1]
                    if metric.startswith("leaderboard_")
...
def main():
    ...
    print(
        f"Hugging Face Open LLM Leaderboard: {len(new_rows)} observations; {len(existing)} total evidence records"
    )
