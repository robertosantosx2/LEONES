...
RAM_TARGETS = [
    ("ram", "memory", f"DDR4 {gb}GB") for gb in (2, 4, 8, 16, 32, 64, 128)
] + [("ram", "memory", f"DDR5 {gb}GB") for gb in (8, 16, 32, 64, 128)]
...
def main():
    ...
    ram_present = {
        (
            r.get("component_type", ""),
            r.get("vendor", ""),
            r.get("category", "") + " " + r.get("capacity_gb", "") + "GB",
        )
        for r in rows
    }
    ...
    if missing_cpu:
        print("CPU sin precio observado:")
        [print(" -", x) for x in missing_cpu]
    if missing_ram:
        print("RAM sin precio observado:")
        [print(" -", x) for x in missing_ram]
    if len(sources) < 2:
        print(
            "WARNING: menos de dos fuentes aportan observaciones; el precio de mercado puede tener baja robustez."
        )
    if newest and newest < date.today().isoformat():
        print(
            "INFO: el histórico vigente no contiene observaciones de hoy; puede ser normal si las fuentes no publicaron novedades."
        )
