...
def main()->int:
 p=argparse.ArgumentParser(description='Revisa un resultado LEONES antes de compartirlo')
 p.add_argument('report'); p.add_argument('--explain',action='store_true')
 a=p.parse_args()
 if a.explain or True: print('🦁 LEONES · Puerta de privacidad\nVoy a buscar patrones habituales de datos personales, identificadores y secretos. No modificaré ni publicaré el fichero.\nUn OK no demuestra anonimato: todavía debes revisar el documento.\n')
 try:text=Path(a.report).read_text(encoding='utf-8',errors='replace')
 except OSError as exc: print(f'No se pudo leer el fichero: {exc}'); return 2
 findings=[name for name,pattern in PATTERNS.items() if re.search(pattern,text)]
 status='blocked' if findings else 'clear'
 out={'schema_version':'1.0','tool':'leones-privacy','tool_version':'1.1','status':status,'findings':findings,'limits':['pattern-based check only','manual review still required','absence of findings is not proof of anonymity'],'next_step':'Revisar manualmente y continuar con publish' if not findings else 'Eliminar/revisar los hallazgos antes de compartir'}
