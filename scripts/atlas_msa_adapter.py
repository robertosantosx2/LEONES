...
def main():
    ...
    for metric, pattern in PATTERNS:
        for m in re.finditer(pattern, body, re.I):
            ...
            rows.append({"model_id":"","model_name":"","source_type":"msa","source_url":URL,"retrieved_at":now,"claim":context,"metric":metric,"value":m.group("value"),"unit":m.group("unit"),"benchmark":"","hardware":"","runtime":"","quantization":"","workload":"local inference","evidence_status":"reported","source_record_id":f"msa:{rid}","extraction_method":"msa_public_page"})
    existing=[]
    if OUT.exists():
        with OUT.open(encoding="utf-8",newline="") as f: existing=list(csv.DictReader(f))
    seen={r.get("source_record_id") for r in existing}
    for r in rows:
        if r["source_record_id"] not in seen: existing.append(r); seen.add(r["source_record_id"])
