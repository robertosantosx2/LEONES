...
def select_from_profile(
    ...
    *,
    ...
    return select(
        rows,
        workload=workload,
        hardware=hardware_label(profile),
        ram_gb=memory_gb,
        vram_gb=0,
...
def main() -> int:
    ...
    rows = list(
        __import__("csv").DictReader(args.feed.open(encoding="utf-8-sig", newline=""))
    )
    llmfit = (
        json.loads(args.llmfit.read_text(encoding="utf-8")) if args.llmfit else None
    )
    result = select_from_profile(
        profile,
        rows,
    ...
    print(
        f"eligible={result['counts']['eligible']} top_n={result['counts']['top_n']} -> {args.out}"
    )
    return 0
