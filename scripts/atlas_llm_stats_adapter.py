...
def main():
    if not IN.exists():
        print(f"No structured LLM Stats input yet: {IN}")
        return
    rows = []
    existing = []
    if OUT.exists():
    ...
    for r in rows:
        ...
        key = "|".join([model, benchmark, metric, value, r.get("setting", "")])
        rid = "llm_stats:" + hashlib.sha256(key.encode()).hexdigest()[:16]
        if rid in seen:
            continue
        existing.append(
            {
    ...
    print(
        f"LLM Stats: {added} new observations; {len(existing)} total evidence records"
    )
