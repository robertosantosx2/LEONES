...
def normalize_candidate(item: dict[str, Any], estimator: str) -> dict[str, Any] | None:
    model_id = (
        item.get("model_id") or item.get("model") or item.get("id") or item.get("name")
    )
    category = _category(item)
    total_m = _params_m(item, False)
    if not model_id or category is None or total_m is None or total_m <= 0:
        return None
    moe = _is_moe(item)
    active_m = _params_m(item, True) if moe else None
    if moe and (active_m is None or active_m <= 0 or active_m > total_m):
        return None
    return {
        **item,
        "model_id": model_id,
        "category": category,
        "is_moe": moe,
        "total_parameters_m": total_m,
        "active_parameters_m": active_m,
        "selection_parameters_m": active_m if moe else total_m,
        "parameter_selection_basis": "active_parameters_m"
        if moe
        else "total_parameters_m",
        "estimator": estimator,
...
def validate_estimator_output(source: str, payload: Any) -> dict[str, Any]:
            ...
            invalid.append(
                {
                    ...
                    or item.get("name"),
                    "category": _category(item),
                    "reason": "missing/invalid parameters/category; MoE requires valid active_parameters",
                }
            )
            continue
    ...
    return {
        "source": source,
        "required_candidates_per_category": ESTIMATOR_CANDIDATE_COUNT,
        "categories": counts,
        "returned_candidates": sum(counts.values()),
        "valid": valid,
...
def select_three_by_parameters(
...
def reduce_estimator_outputs(sources: dict[str, Any] | None = None) -> dict[str, Any]:
    sources = sources or {}
    validation = {
        source: validate_estimator_output(source, sources.get(source, {}))
    ...
    for category in CATEGORIES:
        ...
        for result in validation.values():
            if result["valid"]:
                candidates.extend(result["candidates"][category])
        selected_by_category[category] = select_three_by_parameters(candidates)
    return {
        "required_estimators": len(SOURCES),
        "categories": list(CATEGORIES),
        "required_per_estimator_per_category": 6,
        "expected_external_candidates": EXPECTED_EXTERNAL_CANDIDATES,
        "selected_per_category": 3,
        "selected_total": 9,
        "selection_policy": "per category: smallest + lower-middle + largest; Dense=total_parameters_m, MoE=active_parameters_m",
...
def build_consensus(
    model_id: str, sources: dict[str, Any] | None = None
) -> dict[str, Any]:
    sources = sources or {}
    observations = {}
    values = []
    for source in SOURCES:
        item = next(
            (
                x
                for x in _items(sources.get(source))
                if _norm(
                    x.get("model_id") or x.get("model") or x.get("id") or x.get("name")
                )
                == _norm(model_id)
            ),
            None,
        ...
        text = _norm(value)
        if isinstance(value, bool):
            values.append("fit" if value else "no_fit")
        elif text in {"fit", "good", "yes", "compatible", "can run", "runs"}:
            values.append("fit")
        elif text in {
            "no fit",
            "impossible",
