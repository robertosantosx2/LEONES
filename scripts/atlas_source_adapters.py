...
def record(source_type: str, url: str, claim: str, **kw) -> Evidence:
    ...
    return Evidence(source_type=source_type, source_url=url, retrieved_at=now,
                    claim=claim, source_record_id=f"{source_type}:{rid}", **kw)

class HuggingFaceAdapter:
    name = "hugging_face"
    url = "https://huggingface.co/spaces/open-llm-leaderboard/open_llm_leaderboard"
    def normalize(self, model_name: str, benchmark: str, metric: str, value: str,
                  precision: str = "") -> Evidence:
        return record(self.name, self.url,
            f"{model_name}: {benchmark} {metric}={value}" + (f" ({precision})" if precision else ""),
            model_name=model_name, benchmark=benchmark, metric=metric, value=value,
            unit="score", quantization=precision)

class LMSEvalAdapter:
    name = "lm_arena"
    url = "https://chat.lmsys.org/"
    def normalize(self, model_name: str, rank_or_rating: str, metric: str = "rating") -> Evidence:
        return record(self.name, self.url, f"{model_name}: {metric}={rank_or_rating}",
                      model_name=model_name, metric=metric, value=rank_or_rating,
                      unit="rating" if metric.lower() != "rank" else "rank")

class ArtificialAnalysisAdapter:
    name = "artificial_analysis"
    url = "https://artificialanalysis.ai/"
    def normalize(self, model_name: str, metric: str, value: str, unit: str,
                  provider_or_hardware: str = "") -> Evidence:
        return record(self.name, self.url, f"{model_name}: {metric}={value} {unit}",
                      model_name=model_name, metric=metric, value=value, unit=unit,
                      hardware=provider_or_hardware)

class MSAAdapter:
    name = "msa"
    url = "https://msa.millaguie.net/"
    def normalize(self, model_name: str, claim: str, metric: str = "",
                  value: str = "", unit: str = "", hardware: str = "",
                  runtime: str = "", quantization: str = "") -> Evidence:
        return record(self.name, self.url, claim, model_name=model_name,
                      metric=metric, value=value, unit=unit, hardware=hardware,
                      runtime=runtime, quantization=quantization)

class BenchmarkAdapter:
    def __init__(self, name: str, url: str):
        self.name, self.url = name, url
    def normalize(self, model_name: str, benchmark: str, metric: str,
                  value: str, workload: str = "") -> Evidence:
        return record(self.name, self.url, f"{model_name}: {benchmark} {metric}={value}",
                      model_name=model_name, benchmark=benchmark, metric=metric,
                      value=value, unit="score", workload=workload)
...
def write(rows: list[Evidence]) -> None:
    ...
    for row in rows:
        ...
        if (d["source_type"], d["source_record_id"]) not in seen:
            existing.append(d); seen.add((d["source_type"], d["source_record_id"]))
...
if __name__ == "__main__":
    ...
    adapters = [HuggingFaceAdapter(), LMSEvalAdapter(), ArtificialAnalysisAdapter(), MSAAdapter(),
