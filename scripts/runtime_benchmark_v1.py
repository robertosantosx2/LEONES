...
def begin(
    plan: dict[str, Any], *, protocol: dict[str, Any] | None = None
) -> dict[str, Any]:
...
def complete(record: dict[str, Any], measured: dict[str, Any]) -> dict[str, Any]:
    ...
    numeric_measurements = [
        ...
        if key != "estimated_tps"
        and isinstance(value, (int, float))
        and not isinstance(value, bool)
    ]
    if not numeric_measurements:
        raise ValueError("at least one numeric measured runtime fact is required")
    if "measured_tps" in measured and (
        not isinstance(measured["measured_tps"], (int, float))
        or isinstance(measured["measured_tps"], bool)
        or measured["measured_tps"] < 0
    ):
