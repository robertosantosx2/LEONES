...
PRIVACIDAD: el JSON público no contiene la ruta local del archivo. La ruta se
puede mostrar únicamente en la salida humana local.
"""
...
def main():
    p = argparse.ArgumentParser(
        description="Identifica un archivo de modelo sin ejecutarlo"
    )
    ...
    if not path.is_file():
        print(
            f"ERROR: no existe un archivo regular en: {path}\nSiguiente paso: comprueba la ruta y vuelve a intentarlo."
        )
        return 2
    if a.explain and not a.json:
        print(
            "🦁 LEONES · Identificación de modelo\nVoy a leer el archivo para obtener tamaño, extensión y SHA-256. No ejecutaré el modelo ni lo publicaré.\n"
        )
