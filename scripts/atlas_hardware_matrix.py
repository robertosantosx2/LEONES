...
FIELDS = [
    'cpu_family', 'cpu_name', 'ram_gb', 'gpu_id', 'gpu_name', 'vram_gb',
    'workload', 'rank', 'model_id', 'model_name', 'variant', 'quantization',
    'runtime', 'estimated_memory_gb', 'context_tokens', 'context_target_tokens',
    'tokens_per_second', 'quality_score', 'jgb_level', 'jgb_confidence',
    'fit_score', 'confidence', 'reason'
]
...
def diagnose(rows, hardware, ram, vram, context):
    ...
    limit = ram + vram
    counts = {k: 0 for k in (
        'not_profile', 'memory', 'context', 'runtime',
        'quantization_or_weights', 'hardware', 'workload', 'fits'
    )}
    t23 = 0
    ...
    for r in rows:
        ...
        if not (r.get('runtime') or '').strip():
            counts['runtime'] += 1
            continue
        if not ((r.get('quantization') or '').strip() or (r.get('weight_memory_gb') or '').strip()):
        ...
        for cpu, cpu_name in CPUS:
            for ram in RAMS:
                ...
                for gid, gname, vram in targets:
                    ...
                    cmd = [
                        'python3', str(RECOMMENDER), '--workload', 'chat',
                        '--hardware', hardware, '--ram', str(ram),
                        '--vram', str(vram), '--context', '1', '--out', str(out)
                    ]
                        ...
                        for r in csv.DictReader(generated):
                            ...
                            rows.append({
                                ...
                                **{
                                    ...
                                    for k in FIELDS
                                    if k not in {
                                        'cpu_family', 'cpu_name', 'ram_gb',
                                        'gpu_id', 'gpu_name', 'vram_gb', 'workload'
                                    }
                                },
                            })
