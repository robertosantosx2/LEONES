...
import curses,json,shutil,subprocess,sys
...
def machine_state_screen(s,language):
    while True:
        ...
        if h<25 or w<92:put(s,1,2,tr(language,'small'),w-4);put(s,3,2,tr(language,'resize'),w-4);s.refresh();
        else:
            ...
            except (OSError,KeyError,ValueError):put(s,3,x,"MEMORIA   UNKNOWN",w-8)
            try:
                load=os.getloadavg()[0];cores=os.cpu_count() or 1;put(s,8,x,f"CPU       carga 1m={load:.2f} · {load*100/cores:.1f}% de capacidad lógica ({cores} CPUs)",w-8)
            ...
            put(s,12,x,"TOP 5 PROCESOS · MEMORIA",w-8);put(s,13,x,"PID       %MEM   RSS        PROCESO",w-8)
            put(s,14,x,"(consulta dinámica al entrar; ver detalle en terminal)",w-8)
            try:
                ...
                for i,line in enumerate(out):put(s,15+i,x,line,w-8)
            except (OSError,subprocess.TimeoutExpired):put(s,15,x,"UNKNOWN",w-8)
            put(s,h-7,x,"TOP 5 PROCESOS · CPU",w-8)
            try:
                ...
                for i,line in enumerate(out):put(s,h-6+i,x,line,w-8)
            except (OSError,subprocess.TimeoutExpired):put(s,h-6,x,"UNKNOWN",w-8)
...
def draw(s,phase,purposes,models,selected,solution,cursor,nav_focus,nav_index,language):
    ...
    add_box(s,1,rx,top,rw,tr(language,'status') if 'status' in T.get(language,{}) else 'ESTADO DEL SISTEMA');add_box(s,by,rx,mh,rw,"ESPACIO DE TRABAJO RC4");x=rx+3;cw=rw-6
    ...
    if phase==0:
        ...
        put(s,row,x,"SELECCIONA LA SOLUCIÓN · FUNCIONALIDAD Y CONSUMO HABITUAL",cw);row+=2;cat=catalog()
        for i,(key,name) in enumerate(SOLUTIONS):
            info=cat.get(key,{});put(s,row,x,(">" if key==solution else " ")+name,cw);row+=1
            put(s,row,x,"FUNCIONES: "+", ".join(info.get('functions',[])),cw);row+=1
            put(s,row,x,"USO: "+str(info.get('usage_profile','UNKNOWN')),cw);row+=1
            put(s,row,x,f"INSTALACIÓN: DISCO={human_bytes(info.get('disk_bytes'))} RAM={human_bytes(info.get('ram_bytes'))} CPU={info.get('cpu_load','UNKNOWN')} VRAM={human_bytes(info.get('vram_bytes'))}",cw);row+=2
        ...
        cat=catalog()
        for k in solution_keys(solution):
            info=cat.get(k,{});put(s,row,x,f"{info.get('name',k)}: DISCO={human_bytes(info.get('disk_bytes'))} RAM={human_bytes(info.get('ram_bytes'))} CPU={info.get('cpu_load','UNKNOWN')} VRAM={human_bytes(info.get('vram_bytes'))}",cw);row+=1
