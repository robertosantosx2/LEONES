...
def machine_screen(s,lang):
 while True:
  ...
  put(s,9,x,f"{tr(lang,'top_mem')} [{tr(lang,'measured')}]",w-8)
  for i,line in enumerate(d['memtop']):put(s,10+i,x,line,w-8)
  put(s,16,x,f"{tr(lang,'top_cpu')} [{tr(lang,'measured')}]",w-8)
...
def draw(s,phase,purposes,models,sel,sol,cursor,navfocus,navidx,lang):
 ...
 title='LEONES // AI OPERATING SYSTEM v4';put(s,0,max(2,(w-len(title))//2),title,w-4);put(s,2,3,'+'+'-'*(w-8)+'+',w-8)
 labels=[tr(lang,'purposes'),tr(lang,'models'),tr(lang,'solution'),tr(lang,'cost'),tr(lang,'confirm')];put(s,3,5,'  '.join(('>' if i==phase else ' ')+f'{i+1} {x}' for i,x in enumerate(labels)),w-10);put(s,5,4,tr(lang,'workspace')+' · '+tr(lang,'focus')+': '+('NAVEGACIÓN' if navfocus and lang=='es' else tr(lang,'nav') if navfocus else tr(lang,labels[phase].lower()) if labels[phase] else ''),w-8)
 box(s,7,3,h-10,w-8,tr(lang,'system'));d=machine_data();
 if d['mem']:
  total,free,used=d['mem'];put(s,8,6,f"{tr(lang,'memory')} [{tr(lang,'measured')}]: {hb(total,lang)} {tr(lang,'total').lower()} · {hb(free,lang)} {tr(lang,'free').lower()} · {hb(used,lang)} {tr(lang,'used').lower()}",w-12)
 if d['cpu']:
  load,cores=d['cpu'];put(s,9,6,f"{tr(lang,'cpu')} [{tr(lang,'measured')}]: {load:.2f} {tr(lang,'load')} · {cores} {tr(lang,'logical')}",w-12)
 if d['disk']:put(s,10,6,f"{tr(lang,'disk')} [{tr(lang,'measured')}]: {hb(d['disk'].free,lang)} {tr(lang,'free').lower()} / {hb(d['disk'].total,lang)} {tr(lang,'total').lower()}",w-12)
 row=12
 if phase==0:
  put(s,row,6,tr(lang,'purpose_prompt'),w-12);row+=2
  for i,p in enumerate(PURPOSES):put(s,row+i,8,('>' if i==cursor else ' ')+(' [X] ' if p in purposes else ' [ ] ')+tr(lang,p),w-16)
 elif phase==1:
  put(s,row,6,tr(lang,'models_prompt'),w-12);row+=2
  for i,m in enumerate(models):put(s,row+i,8,('>' if i==cursor else ' ')+('[X] ' if i in sel else '[ ] ')+f"{str(m.get('model_id','?'))[:48]}  DISCO={hb(model_cost(m),lang)}",w-16)
 elif phase==2:
  put(s,row,6,tr(lang,'solution_prompt'),w-12);row+=2
  for i,k in enumerate(SOLUTIONS):put(s,row+i,8,('>' if i==cursor else ' ')+tr(lang,k),w-16)
  _,desc,func,usage=solution_info(sol,lang);put(s,row+5,6,desc,w-12);put(s,row+7,6,tr(lang,'functions')+': '+'; '.join(func),w-12);put(s,row+9,6,tr(lang,'usage')+': '+usage,w-12)
 elif phase==3:
  required=agg(models,sel,sol);free=d['disk'].free if d['disk'] else None;status=tr(lang,'sufficient') if required is not None and free is not None and free>=required else tr(lang,'insufficient') if required is not None and free is not None else tr(lang,'unknown')
  put(s,row,6,tr(lang,'cost'),w-12);row+=2
  for i in sorted(sel):put(s,row,8,f"{models[i].get('model_id','?')}: {hb(model_cost(models[i]),lang)} [{tr(lang,'declared') if model_cost(models[i]) is not None else tr(lang,'unknown')}]");row+=1
  put(s,row+1,6,f"MODELOS + {tr(lang,'solution')}: {hb(required,lang)} · {tr(lang,'disk')} {status}",w-12);put(s,row+3,6,f"{tr(lang,'disk')} {tr(lang,'free').lower()}: {hb(free,lang)} [{tr(lang,'measured')}]")
 elif phase==4:
  put(s,row,6,tr(lang,'confirm'),w-12);row+=2;put(s,row,8,f"{tr(lang,'purpose_prompt')} {', '.join(tr(lang,p) for p in purposes)}",w-16);row+=1;put(s,row,8,tr(lang,'models_count',n=len(sel)),w-16);row+=1;put(s,row,8,f"{tr(lang,'solution')}: {tr(lang,sol)}",w-16);row+=2;put(s,row,8,tr(lang,'consent'),w-16);row+=2;put(s,row,8,tr(lang,'no_install'),w-16)
 put(s,h-2,3,tr(lang,'move')+' · '+tr(lang,'space')+' · '+tr(lang,'enter')+' · '+tr(lang,'back')+' · '+tr(lang,'quit'),w-6);s.refresh()
def main():
 def app(s):
  curses.curs_set(0);s.keypad(True);lang=language_screen(s);machine_screen(s,lang);phase=0;purposes=[];models=[];sel=set();sol='personal_assistant';cursor=0
  while True:
   draw(s,phase,purposes,models,sel,sol,cursor,False,0,lang);k=s.getch()
   ...
   if phase==0:
    ...
    if models:
     if k in (curses.KEY_UP,ord('k')):cursor=(cursor-1)%len(models)
     elif k in (curses.KEY_DOWN,ord('j')):cursor=(cursor+1)%len(models)
     elif k==ord(' '):sel.remove(cursor) if cursor in sel else sel.add(cursor)
    ...
    if k in (curses.KEY_UP,ord('k')):cursor=(cursor-1)%3;sol=SOLUTIONS[cursor]
    elif k in (curses.KEY_DOWN,ord('j')):cursor=(cursor+1)%3;sol=SOLUTIONS[cursor]
    elif k in (10,13):phase=3;cursor=0
    elif k in (ord('b'),ord('B')):phase=1;cursor=0
   elif phase==3:
    if k in (ord('b'),ord('B')):phase=2
    elif k in (10,13):phase=4;cursor=0
   else:
    if k in (ord('b'),ord('B')):phase=3
    elif k in (10,13):pass
