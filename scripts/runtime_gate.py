...
def resolve_runtime(candidate: dict[str, Any], *, available_runtimes: set[str] | None = None,
    ...
    entry = get_runtime(runtime_name); adapter = get_adapter(entry.id)
    ...
    # Authorization is a generic property of having a trusted runner binding, never of runtime identity.
    execution_authorized = bool(entrypoint)
    hw = dict(hardware or {}); hw.update(candidate.get("hardware") or {}); model = candidate.get("model") or {}
    ...
    spec = adapter.prepare(runtime_plan, entry); llmfit = candidate.get("llmfit") or {}
