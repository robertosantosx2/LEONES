...
def resolve_runtime(candidate: dict[str, Any], *, available_runtimes: set[str] | None = None,
    ...
    execution_authorized = entry.entrypoint["kind"] in {"executable", "service", "python-module", "service-or-library", "adapter-controlled"}
    ...
    # The adapter is the only component allowed to validate runtime-specific capabilities.
    adapter.prepare(runtime_plan, entry)
    ...
    return {
        ...
        "parameters": {
            "total_parameters_m": model.get("total_params_m"),
            "active_parameters_m": model.get("active_params_m"),
        ...
        "runtime": {"name": entry.id, "adapter": adapter.adapter_id, "entrypoint": entrypoint,
                    "entrypoint_kind": entry.entrypoint["kind"], "version": candidate.get("runtime_version")},
        "optimization_families": candidate.get("optimization_families") or [],
        "quantization": quantization,
        "hardware": {"ram_gb": hw.get("ram_gb", candidate.get("memory_available_gb") or 0),
                     "os": hw.get("os", "unknown"), "cpu": hw.get("cpu"), "gpu": hw.get("gpu"),
                     "vram_gb": hw.get("vram_gb"), "host_memory_bandwidth_gbps": hw.get("host_memory_bandwidth_gbps"),
...
def gate_selection(selection: dict[str, Any], *, available_runtimes: set[str] | None = None,
    ...
    return {"schema_version": SCHEMA_VERSION, "gate": "LEONES-runtime-selection-gate",
            "execution_plans": plans, "blocked": blocked,
