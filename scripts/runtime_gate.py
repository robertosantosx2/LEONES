...
def resolve_runtime(candidate: dict[str, Any], *, available_runtimes: set[str] | None = None,
    ...
    plan = {"schema_version": SCHEMA_VERSION, "category": candidate.get("category"),
        ...
        "model_id": model_id, "model": {"id": model_id, "name": model_name, "revision": candidate.get("revision")},
        ...
        "hardware": {"ram_gb": hw.get("ram_gb", candidate.get("memory_available_gb") or 0), "os": hw.get("os", "unknown"),
                     "cpu": hw.get("cpu"), "gpu": hw.get("gpu"), "vram_gb": hw.get("vram_gb"),
                     "host_memory_bandwidth_gbps": hw.get("host_memory_bandwidth_gbps"), "pcie_h2d_bandwidth_gbps": hw.get("pcie_h2d_bandwidth_gbps"),
