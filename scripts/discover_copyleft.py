...
def load_osi_licenses():
    ...
    if missing:
        raise RuntimeError('OSI parser missing priority licenses: ' + ', '.join(sorted(missing)))
    return licenses
...
for template in DISCOVERY_QUERIES:
    ...
    try:
        data = github_api('/search/repositories?' + urllib.parse.urlencode({
            'q': query,
            'sort': 'updated',
            'order': 'desc',
            'per_page': 30,
...
items.sort(key=lambda item: (not item['priority_copyleft'], item['updated']), reverse=False)
...
if failed_queries:
    lines.extend(['', '## Query diagnostics', '', f'{len(failed_queries)} queries failed; successful queries were retained.', ''])
