...
def structured_calls(message: dict[str, Any], model: str) -> list[dict[str, Any]]:
    ...
    if canonical[0]["arguments"].get("model_id") != model or canonical[1]["arguments"].get("path") != "report.txt":
        raise RuntimeError("structured A01 arguments do not match the selected model and report path")
    return canonical
...
def main() -> int:
    ...
    if calls:
        ...
        fallback = post_json(args.url, {"model": args.model, "messages": [{"role": "system", "content": "Execute LEONES A01. Return exactly two tool calls: lookup_model first with model_id exactly " + args.model + ", then write_report with path exactly report.txt. No prose."}, {"role": "user", "content": args.prompt}], "format": schema, "stream": False, "options": {"temperature": 0}}, args.timeout)
    ...
    if canonical[0].get("arguments", {}).get("model_id") != args.model or canonical[1].get("arguments", {}).get("path") != "report.txt":
        raise RuntimeError("A01 tool arguments do not match the selected model and report path")

    print(json.dumps(canonical[0], ensure_ascii=False))
    print(json.dumps(canonical[1], ensure_ascii=False))
    print(json.dumps({"measured_tps": round(total_eval_tokens / (total_eval_seconds / 1_000_000_000), 4)} if total_eval_tokens > 0 and total_eval_seconds > 0 else {"measurement_status": "runtime_value_not_reported"}, ensure_ascii=False))
