...
(() => {
    ...
    function renderNavigation() {
        ...
        if (currentPage?.[2] && currentPage[2] !== "top") {
            const sep = document.createElement("span"); sep.textContent = "›"; ci.appendChild(sep);
        ...
        const sep2 = document.createElement("span"); sep2.textContent = "›"; ci.appendChild(sep2);
        const cur = document.createElement("strong"); cur.textContent = currentPage?.[0] || document.title; ci.appendChild(cur);
        crumb.appendChild(ci); root.appendChild(crumb);
        const toggle = document.createElement("button");
        toggle.className = "leones-nav-toggle"; toggle.type = "button";
        toggle.setAttribute("aria-controls", "leones-side"); toggle.setAttribute("aria-expanded", "false");
        toggle.textContent = "☰ Menú"; root.appendChild(toggle);
        ...
        const bl = document.createElement("a"); bl.href = "index.html"; bl.setAttribute("aria-label", "LEONES · Inicio");
        const bi = document.createElement("img"); bi.src = base; bi.alt = "LEONES"; bi.width = 112; bi.height = 62; bl.appendChild(bi); brand.appendChild(bl); side.appendChild(brand);
        ...
        root.appendChild(side); document.body.prepend(root);
        ...
        backdrop.addEventListener("click", close); side.addEventListener("click", (e) => { if (e.target.closest("a")) close(); });
