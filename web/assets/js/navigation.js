...
(() => {
    "use strict";
    const navigation = [
        ["Inicio", "index.html", "top"], ["Proyectos", "proyectos.html", "top"], ["Atlas", "atlas.html", "project"],
        ["Pilares", "pilares.html", "project"], ["Arquitectura", "arquitectura.html", "project"], ["Diagramas", "diagramas.html", "project"],
        ["Pila", "pila.html", "project"], ["Operación", "operacion.html", "project"], ["Aplicación", "app.html", "top"],
        ["Scripts", "scripts.html", "application"], ["Resultados", "resultados.html", "application"], ["Evaluación", "evaluacion.html", "application"],
        ["Recomendaciones", "recommendations.html", "application"], ["Recomendaciones de visita", "recomendaciones-visita.html", "application"],
        ["Recomendar a LEONES", "recomendar.html", "application"], ["Manada", "manada.html", "top"], ["Prospección", "prospeccion.html", "top"],
        ["Horizonte", "horizon.html", "top"], ["Contacto", "contacto.html", "top"]
    ];
    const identity = {
        prospeccion: ["🔎", "Prospección"], prospection: ["🔎", "Prospección"], atlas: ["▦", "Atlas"], task: ["◉", "Task Intelligence"],
        router: ["↗", "Router"], quant: ["⚖", "Quant"], finetune: ["⚙", "Fine-Tuning"], hardware: ["▣", "Hardware"],
        modelos: ["◆", "Modelos"], inferencia: ["▶", "Inferencia"], agents: ["⚒", "Agents"], evidencia: ["✓", "Evidencia"],
        privacidad: ["◆", "Privacidad"], publicacion: ["↑", "Publicación"], manada: ["♟", "Manada"], stats: ["▥", "Stats"],
        herramientas: ["⚒", "Herramientas"], arquitectura: ["⌘", "Arquitectura"], pilares: ["✦", "Pilares"], acerca: ["i", "Acerca de"]
    };
    ...
    function createLink([label, path, level]) { const link=document.createElement("a"); link.className=`level-${level}`; link.href=path; link.textContent=label; if(path===currentPath){link.classList.add("active");link.setAttribute("aria-current","page");} return link; }
    function roleFromSrc(src="") { const m=src.match(/leones-([a-z0-9-]+)\.svg/i); return m ? m[1].replace("-","_") : ""; }
    function decorateImage(img) {
        ...
        const wrap=document.createElement("span"); wrap.className="leones-composite-icon"; wrap.setAttribute("aria-label",data[1]);
        const badge=document.createElement("span"); badge.className="leones-composite-badge"; badge.textContent=data[0]; badge.setAttribute("aria-hidden","true");
        img.parentNode.insertBefore(wrap,img); wrap.appendChild(img); wrap.appendChild(badge);
    }
    function decorateExistingLogos() { document.querySelectorAll('img[src*="/graphics/logos/"], img[src*="assets/graphics/logos/"]').forEach(decorateImage); }
    function renderNavigation() {
        ...
        const bi=document.createElement("img");bi.src=base;bi.alt="LEONES";bi.width=72;bi.height=72;bi.className="leones-sidebar-face";bl.appendChild(bi);brand.appendChild(bl);side.appendChild(brand);
        const title=document.createElement("div");title.className="side-title";title.textContent="Navegación";side.appendChild(title);navigation.forEach(item=>side.appendChild(createLink(item)));root.appendChild(side);document.body.prepend(root);
    ...
    function boot(){renderNavigation();decorateExistingLogos();}
