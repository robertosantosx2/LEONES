...
selection. It never measures performance and never authorizes execution.
"""
...
def build_candidate_set(
    ...
    *,
    ...
    Candidate facts are retained as proposals. Missing values remain null;
    estimated performance is never converted into a LEONES measurement.
    """
...
def validate_candidate_set(payload: dict[str, Any]) -> None:
    ...
    forbidden = {"tokens_per_second", "measured_tps", "benchmark_result", "command", "argv"}
    for candidate in payload["candidates"]:
        leaked = forbidden.intersection(candidate)
        if leaked:
            raise ValueError(f"candidate contains execution/measurement fields: {sorted(leaked)}")
