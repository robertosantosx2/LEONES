...
def test_preflight_accepts_rootful_docker_without_rootless_detection():
    def run(command, timeout=5):
        ...
        return 1, ""
    with patch.object(ods_preflight.shutil, "which", side_effect=lambda n: "/usr/bin/" + n if n == "docker" else None), patch.object(ods_preflight, "run_command", side_effect=run):
...
def test_preflight_recognizes_podman_without_falsely_claiming_ods_compatibility():
    def run(command, timeout=5):
        ...
        if cmd == ("podman", "info", "--format", "{{.Host.Security.Rootless}}"):
            return 0, "true"
        return 1, ""
    with patch.object(ods_preflight.shutil, "which", side_effect=lambda n: "/usr/bin/" + n if n == "podman" else None), patch.object(ods_preflight, "run_command", side_effect=run):
        result = ods_preflight.detect_container_runtime()
...
def test_physical_verification_accepts_sudo_docker():
    def run(command, timeout=8.0):
        # Simulate the real failure mode: direct Docker daemon access fails,
        # while the same daemon is reachable through sudo.
        if command == ["docker", "info"]:
            return 1, "permission denied"
        if command == ["sudo", "docker", "info"]:
            return 0, ""
        if command[-1] == "--version":
            return 0, "Docker version 29"
        if command[-2:] == ["compose", "version"]:
            return 0, "Docker Compose version v2"
        if command[-1] == "{{.Repository}}:{{.Tag}}":
            return 0, "ods:latest"
        return 1, ""
    with patch.object(verify_physical, "_which", side_effect=lambda n: "/usr/bin/" + n if n in {"docker", "sudo"} else None), patch.object(verify_physical, "_run", side_effect=run):
