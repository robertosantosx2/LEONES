from scripts.runtime_registry import SCHEMA_VERSION, registry_entries, capability_match
from scripts.runtimes.v1_1_adapters import ADAPTERS, get_adapter
    ...
    for runtime, adapter in ADAPTERS.items():
        ...
        if runtime == "FreeToken":
            p.update({"architecture_class": "moe", "backend": "cuda",
                      "model": {"total_params_b": 7, "quantized_weight_gb": 4},
                      "moe": {"is_moe": True}, "hardware": {"vram_gb": 24, "ram_gb": 32}})
        adapter.prepare(p, entry)


def test_capability_match_blocks_incompatible_runtime():
    entry = registry_entries()["TensorRT-LLM"]
    ok, reasons = capability_match(entry, backend="metal")
    assert not ok
    assert reasons
