...
def plan(runtime, **extra):
    value = {"runtime": runtime, "model_id": "org/model",
             "model": {"total_params_m": 7000, "active_params_m": 7000},
             "quantization": "Q4_K_M", "architecture_class": "dense",
             "execution_authorized": True, "optimization_families": [],
             "hardware": {}, "workload": {}}
    value.update(extra)
    return value
...
def test_every_adapter_accepts_a_controlled_plan():
    ...
    for runtime, adapter in ADAPTERS.items():
        ...
        if runtime == "FreeToken":
            p.update({"architecture_class": "moe", "backend": "cuda",
                      "model": {"total_params_b": 70, "quantized_weight_gb": 30},
                      "moe": {"is_moe": True}, "workload": {"agentic": True},
                      "hardware": {"vram_gb": 24, "ram_gb": 64,
                                   "host_memory_bandwidth_gbps": 500,
                                   "pcie_h2d_bandwidth_gbps": 20,
                                   "cpu_moe_bandwidth_gbps": 100}})
        adapter.prepare(p, entry)


def test_alias_resolution_is_generic():
...
def test_benchmark_never_turns_estimate_into_measurement():
    record = begin({"execution_authorized": True, "runtime": {"name": "vLLM", "adapter": "vllm.v1.1"},
                    "model_id": "org/model", "model": {}, "quantization": "FP8",
                    "hardware": {}, "workload": {}, "estimated_tps": 123.0})
