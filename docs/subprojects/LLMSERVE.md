...
LLMServe es la capa de **serving local**: descubre modelos locales y backends de inferencia y permite lanzar el modelo seleccionado mediante una TUI. El proyecto upstream se presenta como compañero de LLMFit: LLMFit responde qué modelos encajan en el hardware y LLMServe ejecuta el elegido.

LEONES mantiene las responsabilidades separadas:
...
El upstream documenta detección de llama-server, KoboldCpp, LocalAI, MLX, Ollama, vLLM y LM Studio.
...
2. Mantener un smoke test de presencia del upstream y de su estructura Rust.
...
5. Registrar `modelo + backend + configuración + hardware` antes de medir.
6. Comparar después la predicción de LLMFit con el rendimiento observado por los benchmarks de LEONES.

## Principio de procedencia

LLMServe no es una fuente de verdad sobre calidad de modelos. Es una capa de ejecución/serving. Los datos de rendimiento obtenidos al usarlo son `measured` únicamente cuando LEONES los haya ejecutado y registrado bajo condiciones reproducibles.
