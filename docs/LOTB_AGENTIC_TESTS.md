# LOTB — Pruebas agentivas estándar

LOAS incorpora un primer **smoke test estándar** para simplificar la evaluación inicial de una pila agentic.
...
python3 scripts/lotb-agentic-test.py \
  ...
  --model local-model \
  --output results/lotb/latest.json
```

Si no se indica el modelo, intenta detectar automáticamente el primero anunciado por `/v1/models`.
...
El resultado se guarda en JSON para facilitar su incorporación posterior a metaLOAS.

## Importante: smoke test ≠ LOTB completo

Estas pruebas están diseñadas para responder rápidamente a una primera pregunta:

> **¿Esta configuración local puede ejecutar de forma mínimamente coherente las capacidades agentic básicas de LOTB?**
...
No se debe declarar que una pila «rula» únicamente porque el smoke test sea positivo: también debe cumplir las condiciones CABE y los criterios LOTB correspondientes.
